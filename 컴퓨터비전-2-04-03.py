...
while True:
    ...
    #4.ROI 설정: 추출하고자 하는 차선 영역
    # vertices = np.array([[(50,h),(w/2-45, h/2+60), 
    #                     (w/2+45, h/2+60), (w-50,h)]], dtype=np.int32)
    vertices = np.array([[(50,h-70),(w/2+80, h/2+60), 
                        (w/2+150, h/2+60), (w-50,h-70)]], dtype=np.int32)
    ...
    cv2.imshow('canny_img',canny_img)       # canny_img 이미지 출력
    cv2.imshow('ROI_img',ROI_img)           # ROI_img 이미지 출력
    cv2.imshow('hough_img',hough_img)       # hough_img 이미지 출력
    cv2.imshow('result',result)             # 결과 이미지 출력

    key = cv2.waitKey(int(delay/2))
    if key == 27:
        break
